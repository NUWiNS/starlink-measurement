...
base_dir = os.path.join(DATASET_DIR, 'maine_starlink_trip/nslookup')
...
def plot_cdf_dns_time_with_all_domains(df: pd.DataFrame, output_dir: str):
    domains = sorted(df['req_domain'].unique())
    fig, ax = plt.subplots()
    for domain in domains:
        domain_df = df[df['req_domain'] == domain]['duration_ms']
        filtered_df = filter_outliers_by_IQR(domain_df, threshold=1.5)
        data_stats = create_stats(total_count=len(domain_df), filter_by_count=len(filtered_df))
        stats = get_statistics(filtered_df, data_stats=data_stats)
        # label = format_statistics(stats, unit='ms')
        xvals, yvals = cdf(filtered_df)

        ax.plot(xvals, yvals, label=domain)
    ...
    ax.grid()
    if (output_dir):
        plt.savefig(os.path.join(output_dir, 'cdf_dns_resolve_time_with_all_domains.png'))
    plt.show()
...
def main():
    ...
    plot_cdf_dns_time_with_all_domains(df, output_dir=output_dir)
