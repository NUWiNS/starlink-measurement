...
from scripts.utilities.geo_ip_utils import IGeoIPQuery, JsonGeoIPQuery, MaxMindGeoIPQuery
from scripts.logging_utils import create_logger
from scripts.constants import DATASET_DIR, OUTPUT_DIR

base_dir = os.path.join(DATASET_DIR, "alaska_starlink_trip/traceroute")
...
def plot_hop_points_on_map(geo_ip_query: IGeoIPQuery):
    ...
    m = folium.Map(location=[63.5907, -154.4133], zoom_start=4, tiles=grayscale_tiles, attr=attr)
    ...
    # Save the map
    map_output_path = os.path.join(output_dir, f'traceroute_map_{timestamp}.max.html')
    m.save(map_output_path)
    logger.info(f'\nSaved traceroute map to {map_output_path}')
    
    # Save the IP info map
    ip_info_output_path = os.path.join(output_dir, f'traceroute_map_{timestamp}.max.ip.json')
