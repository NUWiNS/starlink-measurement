...
while true; do
    ...
    echo "Start time: $(date '+%s%3N')">$log_file_name
    nslookup_domain="www.google.com"
    nslookup $nslookup_domain | ts '[%Y-%m-%d %H:%M:%.S]'>>$log_file_name >>$log_file_name
    echo "End time: $(date '+%s%3N')">>$log_file_name
    ...
    traceroute $traceroute_domain | ts '[%Y-%m-%d %H:%M:%.S]'>>$log_file_name >>$log_file_name
    echo "End time: $(date '+%s%3N')">>$log_file_name
